<!DOCTYPE html>
<html>

<head>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="CSS/style.css">
    <title>
        Ticket Price
    </title>

</head>

<body>
    <div class="header1">
        <div class="h">
            <!--Title-->
            <h1><b>Book My Train</b></h1>
            <!--Logo & Login-->
            <img src="Img/log.jpg" class="logo">
            <a href="#"><img src="Logo/lock.png" class="lock"></a>
        </div>
        <!--Navigation-->
        <div class="navi">
            <a class="home1" href="Home.php">Home</a>
            <a href="Timetable.php">Time Table</a>
            <a href="TicketPrice.php">Ticket Price</a>
            <a href="promotions.php">Promotions</a>
            <a href="aboutus.php">About Us</a>
            <a href="ContactUs.php">Contact Us</a>
            <div class="search">
                <form action="/action_page.php">
                    <input type="text" placeholder="Search.." name="search">
                    <button type="submit">Search</button>
                </form>
            </div>
        </div>
    </div>

    <div id="back">
        <div id="toDiv">
            <!--Table Topic-->
            <h2 class="topic"><b><u>Ticket Prices</u></b></h2>
            <p class="classN"><b>(2nd Class)</b></p>
            <p>(All are in "Srilanka Rupees")</p>
        </div>

        <div>
            <!--Price Table-->
            <table class="table1">
                <tr>
                    <th></th>
                    <th>FOT</th>
                    <th>GPH</th>
                    <th>PLG</th>
                    <th>KRN</th>
                    <th>GLM</th>
                    <th>TBM</th>
                    <th>ANP</th>
                    <th>MWH</th>
                    <th>VNA</th>
                    <th>KOC</th>
                    <th>PAL</th>
                    <th>KKM</th>
                    <th>JFN</th>
                </tr>

                <tr>
                    <td>FOT</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>GPH</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>PLG</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>KRN</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>

                </tr>

                <tr>
                    <td>GLM</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>TBM</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>ANP</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>MWH</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>VNA</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>KOC</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>PAL</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>KKM</td>
                    <td>700.00</td>
                    <td>700.00</td>
                    <td>700.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                    <td></td>
                </tr>

                <tr>
                    <td>JFN</td>
                    <td>700.00</td>
                    <td>700.00</td>
                    <td>700.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>550.00</td>
                    <td>480.00</td>
                    <td>400.00</td>
                    <td>400.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td>250.00</td>
                    <td></td>
                </tr>

            </table>
        </div>
        <br/>
        <br/>
        <div>
            <p class="Nlist"><u><b>Name List</b></u></p>
            <table class="table2">
                <!--Name Table-->
                <tr>
                    <th>Short Name</th>
                    <th>Full Name</th>
                </tr>

                <tr>
                    <td>FOT</td>
                    <td>Colombo Fort</td>
                </tr>

                <tr>
                    <td>GPH</td>
                    <td>Gampaha</td>
                </tr>

                <tr>
                    <td>PLG</td>
                    <td>Polgahawela</td>
                </tr>

                <tr>
                    <td>KRN</td>
                    <td>Kurunegala</td>
                </tr>

                <tr>
                    <td>GLM</td>
                    <td>Galgamuwa</td>
                </tr>

                <tr>
                    <td>TBM</td>
                    <td>Tambuttegama</td>
                </tr>

                <tr>
                    <td>ANP</td>
                    <td>Anuradapura</td>
                </tr>

                <tr>
                    <td>MWH</td>
                    <td>Madawachchi </td>
                </tr>

                <tr>
                    <td>VNA</td>
                    <td>Vavuniya</td>
                </tr>

                <tr>
                    <td>KOC</td>
                    <td>Kilinochchi</td>
                </tr>

                <tr>
                    <td>PAL</td>
                    <td>Pallai</td>
                </tr>

                <tr>
                    <td>KKM</td>
                    <td>Kodikamam</td>
                </tr>

                <tr>
                    <td>JFN</td>
                    <td>Jaffna</td>
                </tr>

            </table>
        </div>
        <br/>

        <div class="pagination">
            <a href="TicketPrice.php">&laquo;</a>
            <a href="TicketPrice.php">1</a>
            <a href="TicketPrice2.php" class="active">2</a>
            <a href="TicketPrice3.php">3</a>
            <a href="TicketPrice4.php">4</a>
            <a href="TicketPrice3.php">&raquo;</a>
        </div>
    </div>

    <!--Footer 01-->
    <div class="footer1">
        <div>
            <img src="Logo/phone-book.png" class="contact">
        </div>
        <div class="email">Email :- Contact@Bookmytrain.com </div>
        <div class="social">
            <a href="https://twitter.com"><img src="Logo/twitter.png"></a>
            <a href="https://www.linkedin.com"><img src="Logo/linkedin.png"></a>
            <a href="https://www.facebook.com"><img src="Logo/facebook.png"></a>
            <a href="https://www.instagram.com/"><img src="Logo/instagram.png"></a>
        </div>

        <div>
            <ul id="link">
                <a href="ContactUs.php">
                    <li>Contact Us</li>
                </a>
                <a href="aboutus.php">
                    <li>About Us</li>
                </a>
                <a href="terms.php">
                    <li>Terms & Conditions</li>
                </a>
            </ul>

        </div>

        <div class="payment">
            <img src="Logo/paypal.png">
            <img src="Logo/mastercard.png">
            <img src="Logo/visa.png">
            <img src="Logo/american-express.png">
        </div>
    </div>



</body>

<footer>
    <!--Footer 02-->
    <div class="footer2">
        Copyright &copy; 2019 Book My Train Co.- All rights Reserved
    </div>

</footer>



</html>